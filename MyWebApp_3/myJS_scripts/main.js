...
const CDC_CATEGORIES = [
    {
        ...
        "variables": [
            ...
            {
                "field": "KIDNEY_CrudePrev",
                "label": "Chronic kidney disease",
                "ciField": "KIDNEY_Crude95CI"
            },
            {
    ...
    },
    {
        ...
        "variables": [
            ...
            {
                "field": "CERVICAL_CrudePrev",
                "label": "Cervical cancer screening",
                "ciField": "CERVICAL_Crude95CI"
            },
            {
            ...
            {
                "field": "COREM_CrudePrev",
                "label": "Core preventive services for older men",
                "ciField": "COREM_Crude95CI"
            },
            {
                "field": "COREW_CrudePrev",
                "label": "Core preventive services for older women",
                "ciField": "COREW_Crude95CI"
            },
            {
    ...
    } // End of the four defined categories in the "CDC_CATEGORIES" array
...
*/
const APP_DATA_VERSION = "20260709-eji-environmental-predictors";
const CDC_CSV_PATH = `data/Clean_NM.csv?v=${APP_DATA_VERSION}`; // This path points to the Clean_NM.csv file, which contains the CDC PLACES data for New Mexico. The "CDC_CSV_PATH" constant is used in the code to load and access the CSV data for mapping and analysis.
const NM_TRACT_GEOJSON_PATH = `data/census_tract_county_nm_2020.geojson?v=${APP_DATA_VERSION}`; // This path points to the census tract GeoJSON file for New Mexico, which contains the geographic boundaries of census tracts in the state. The "NM_TRACT_GEOJSON_PATH" constant is used in the code to load and access the GeoJSON data for mapping and spatial analysis.
const NM_COUNTY_GEOJSON_PATH = `data/tl_2018_nm_county.geojson?v=${APP_DATA_VERSION}`; // This path points to the county boundary GeoJSON file for New Mexico, which contains the geographic boundaries of counties in the state. The "NM_COUNTY_GEOJSON_PATH" constant is used in the code to load and access the GeoJSON data for mapping and spatial analysis, particularly for displaying county boundaries as reference layers on the maps.
const LASSO_REGRESSION_RESULTS_PATH = `data/lasso_regression_results.json?v=${APP_DATA_VERSION}`;
...
let lassoRegressionMeasureLabels = {};
...
function normalizeCdcRows(rows) {
    ...
    }).filter(Boolean); // End of the mapping function that processes each raw CSV row and returns an array of normalized row objects.
...
function initializeAllCategoryViews() {
    ...
    CDC_CATEGORIES.forEach(function (category) {
        ...
        if (category.disabled) {
            legend.innerHTML = "<b>No active legend.</b> This category is not available in the Clean_NM CSV.";
            summary.innerHTML = "This row is disabled because its variables are not present in the current dataset.";
            return;
...
function getMeasureLabel(measureField) {
    ...
    if (lassoRegressionMeasureLabels[measureField]) return lassoRegressionMeasureLabels[measureField];
...
function loadLassoRegressionOutput() {
    ...
    d3.json(LASSO_REGRESSION_RESULTS_PATH).then(function (results) {
        ...
        lassoRegressionMeasureLabels = Object.assign({}, results.predictor_labels || {});
...
function renderLassoRegressionModel(results) {
    ...
    const selectedRows = results.selected_variables.map(function (row) {
        const variableLabel = row.label || getMeasureLabel(row.variable);
        return `
            <tr>
