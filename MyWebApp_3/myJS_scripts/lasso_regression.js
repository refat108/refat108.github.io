const LASSO_DATA_VERSION = "20260709-eji-environmental-predictors";
const LASSO_REGRESSION_RESULTS_PATH = `data/lasso_regression_results.json?v=${LASSO_DATA_VERSION}`;
const LASSO_CSV_PATH = `data/Clean_NM.csv?v=${LASSO_DATA_VERSION}`;
const LASSO_EJI_CSV_PATH = `data/EJI_NM.csv?v=${LASSO_DATA_VERSION}`;
const HISTOGRAM_BIN_COUNT = 12;

let lassoMeasureLabels = {
    ...
    "CERVICAL_CrudePrev": "Cervical cancer screening",
    ...
    "COPD_CrudePrev": "COPD",
    "COREM_CrudePrev": "Core preventive services for older men",
    "COREW_CrudePrev": "Core preventive services for older women",
...
function loadLassoRegressionOutput() {
    ...
    Promise.all([
        ...
        d3.csv(LASSO_EJI_CSV_PATH)
    ]).then(function ([results, rows, ejiRows]) {
        lassoMeasureLabels = Object.assign({}, lassoMeasureLabels, results.predictor_labels || {});
        lassoCdcRows = mergeRegressionRows(results, rows, ejiRows);
        results = sanitizeLassoResultsForCsv(results, rows);
...
function mergeRegressionRows(results, cdcRows, ejiRows) {
    const predictorFields = results.candidate_predictor_fields || [];
    const ejiByGeoid = new Map();
    ejiRows.forEach(function (row) {
        const geoid = normalizeTractId(row.GEOID);
        if (geoid) {
            ejiByGeoid.set(geoid, row);
        }
    });

    const mergedRows = cdcRows.map(function (row) {
        const tractFips = normalizeTractId(row.TractFIPS);
        const countyName = String(row.CountyName || "").trim();
        if (!tractFips || !countyName) {
            return null;
        }

        const ejiRow = ejiByGeoid.get(tractFips) || {};
        const mergedRow = Object.assign({}, row);
        predictorFields.forEach(function (field) {
            mergedRow[field] = ejiRow[field];
        });
        return mergedRow;
    }).filter(Boolean);

    mergedRows.columns = Array.from(new Set([].concat(cdcRows.columns || [], predictorFields)));
    return mergedRows;
}

function normalizeTractId(value) {
    const trimmed = String(value || "").trim();
    return trimmed ? trimmed.padStart(11, "0") : "";
}

function sanitizeLassoResultsForCsv(results, rows) {
    const availableFields = new Set((lassoCdcRows && lassoCdcRows.columns) || Object.keys((lassoCdcRows && lassoCdcRows[0]) || {}));
    const models = (Array.isArray(results.models) ? results.models : [results]).filter(function (model) {
        return availableFields.has(model.dependent_variable);
    });

    models.forEach(function (model) {
        model.selected_variables = (model.selected_variables || []).filter(function (row) {
            return availableFields.has(row.variable);
        });
        model.removed_variables = (model.removed_variables || []).filter(function (variable) {
            return availableFields.has(variable);
        });
        model.selected_variable_count = model.selected_variables.length;
        model.removed_variable_count = model.removed_variables.length;
    });

    if (Array.isArray(results.models)) {
        results.models = models;
    }
    results.dependent_variable_options = (results.dependent_variable_options || []).filter(function (option) {
        return availableFields.has(option.field) && models.some(function (model) {
...
function renderLassoRegressionOutput(results) {
    ...
    const output = document.getElementById("lasso-regression-output");
    if (!models.length || !models[0] || !models[0].dependent_variable) {
        if (output) {
            output.innerHTML = `
                <div class="alert alert-warning">
                    <b>No regression models match the current CSV fields.</b>
                    Regenerate <code>data/lasso_regression_results.json</code> from <code>data/Clean_NM.csv</code>.
                </div>`;
        }
        return;
    }
    ...
    output.innerHTML = `
        <div class="lasso-summary-grid">
            <div class="lasso-summary-card">
                <span class="cdc-stat-label">Complete regression rows</span>
                <span class="cdc-stat-value">${results.row_count_used.toLocaleString()} of ${results.row_count_total.toLocaleString()}</span>
            </div>
            <div class="lasso-summary-card">
...
function getMeasureLabel(fieldName) {
    return lassoMeasureLabels[fieldName] || fieldName;
}
