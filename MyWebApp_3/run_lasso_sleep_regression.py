...
EJI_CSV_PATH = ROOT / "data" / "EJI_NM.csv"
EJI_DICTIONARY_PATH = ROOT / "data" / "EJI_DATADICTIONARY_2022.csv"
...
def load_environmental_predictor_metadata() -> tuple[list[str], dict[str, str]]:
    dictionary = pd.read_csv(EJI_DICTIONARY_PATH, skiprows=1, encoding="cp1252")
    environmental_rows = dictionary[
        (dictionary["MODULE"] == "EBM")
        & dictionary["2022 VARIABLE NAME"].astype("string").str.startswith("E_", na=False)
    ].copy()

    predictor_columns = environmental_rows["2022 VARIABLE NAME"].astype(str).tolist()
    predictor_labels = dict(
        zip(
            environmental_rows["2022 VARIABLE NAME"].astype(str),
            environmental_rows["2022 DESCRIPTION"].astype(str),
        )
    )
    return predictor_columns, predictor_labels


ENVIRONMENTAL_PREDICTOR_COLUMNS, ENVIRONMENTAL_PREDICTOR_LABELS = load_environmental_predictor_metadata()
...
def load_modeling_dataframe() -> pd.DataFrame:
    cdc_df = filter_valid_tract_rows(pd.read_csv(CSV_PATH, dtype={"TractFIPS": "string", "CountyFIPS": "string"}))
    cdc_df["TractFIPS"] = cdc_df["TractFIPS"].astype("string").str.strip().str.zfill(11)

    eji_columns = ["GEOID"] + ENVIRONMENTAL_PREDICTOR_COLUMNS
    eji_df = pd.read_csv(EJI_CSV_PATH, usecols=eji_columns, dtype={"GEOID": "string"})
    eji_df["GEOID"] = eji_df["GEOID"].astype("string").str.strip().str.zfill(11)
    for column in ENVIRONMENTAL_PREDICTOR_COLUMNS:
        eji_df[column] = pd.to_numeric(eji_df[column], errors="coerce").replace(-999, np.nan)

    return cdc_df.merge(eji_df, left_on="TractFIPS", right_on="GEOID", how="inner")
...
def run_regression(dependent_variable: str, dependent_variable_label: str, raw_df: pd.DataFrame | None = None) -> dict:
    if raw_df is None:
        raw_df = load_modeling_dataframe()
    ...
    predictor_columns = [col for col in ENVIRONMENTAL_PREDICTOR_COLUMNS if col in raw_df.columns]
    ...
    for name, coef, std_coef in zip(predictor_columns, coefficients, standardized_coefficients):
        row = {
            "variable": name,
            "label": ENVIRONMENTAL_PREDICTOR_LABELS.get(name, name),
    ...
    result = {
        ...
        "predictor_data_file": str(EJI_CSV_PATH.relative_to(ROOT)).replace("\\", "/"),
        "predictor_source": "CDC/ATSDR Environmental Justice Index environmental burden raw estimate variables",
        "method_note": (
            f"Used {dependent_variable} as the dependent variable and used {len(predictor_columns)} "
            "Environmental Justice Index environmental burden raw estimate variables as candidate predictors. "
        ...
        "candidate_environmental_predictor_count": int(len(predictor_columns)),
...
def build_output_payload() -> dict:
    ...
    return {
        ...
        "data_file": str(CSV_PATH.relative_to(ROOT)).replace("\\", "/"),
        "predictor_data_file": str(EJI_CSV_PATH.relative_to(ROOT)).replace("\\", "/"),
        "predictor_dictionary_file": str(EJI_DICTIONARY_PATH.relative_to(ROOT)).replace("\\", "/"),
        "predictor_source": "CDC/ATSDR Environmental Justice Index environmental burden raw estimate variables",
        "predictor_labels": ENVIRONMENTAL_PREDICTOR_LABELS,
        "candidate_predictor_fields": ENVIRONMENTAL_PREDICTOR_COLUMNS,
