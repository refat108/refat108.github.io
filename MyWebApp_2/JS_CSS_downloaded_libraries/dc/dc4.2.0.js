  ...
  function (
    ...
    printers.filter = function (filter) {
      ...
      if (typeof filter !== "undefined" && filter !== null) {
        if (filter instanceof Array) {
          if (filter.length >= 2) {
            s = `[${filter
              .map((e) => utils.printSingleValue(e))
              .join(" -> ")}]`;
    ...
    class BaseMixin {
      ...
      _computeOrderedGroups(data) {
        ...
        return Array.from(data).sort((a, b) =>
          d3Array.ascending(this._ordering(a), this._ordering(b))
        );
      }
    ...
    class CoordinateGridMixin extends ColorMixin(MarginMixin) {
      ...
      _domainToZoomTransform(newDomain, origDomain, xScale) {
        const k =
          (origDomain[1] - origDomain[0]) / (newDomain[1] - newDomain[0]);
    ...
    const d3Box = function () {
      ...
      function box(g) {
        g.each(function (data, index) {
          ...
          const whiskerIndices = whiskers && whiskers.call(this, data, index),
            whiskerData =
              whiskerIndices && whiskerIndices.map((_i) => data[_i]);
      ...
      box.domain = function (x) {
        ...
        domain =
          x === null ? x : typeof x === "function" ? x : utils.constant(x);
        return box;
      };
    ...
    class StackMixin extends CoordinateGridMixin {
      ...
      _prepareValues(layer, layerIdx) {
        ...
        layer.values = this.evadeDomainFilter()
          ? allValues
          : layer.domainValues;
      }
    ...
    class BarChart extends StackMixin {
      ...
      _barHeight(d) {
        return utils.safeNumber(
          Math.abs(this.y()(d.y + d.y0) - this.y()(d.y0))
        );
      }
        ...
        if (this._barWidth === undefined) {
          ...
          if (this.isOrdinal() && this._gap === undefined) {
            ...
            this._barWidth = Math.floor(
              (this.xAxisLength() - (numberOfBars - 1) * this._gap) /
                numberOfBars
            );
          } else {
            this._barWidth = Math.floor(
      ...
      _doRedraw() {
        ...
        let boxes = this._chartBody
          ...
          .data(
            this.data(),
            (d, i) =>
              `${this.keyAccessor()(d, i)}\0${this.valueAccessor()(d, i)}`
          );
    ...
    class Legend {
      ...
      _makeLegendKeyboardAccessible() {
        ...
        const tabElements = this._parent
          .svg()
    ...
    class NumberDisplay extends BaseMixin {
      ...
      constructor(parent, chartGroup) {
        ...
        this.data((group) => {
          const valObj = group.value
            ? group.value()
            : this._maxBin(group.all());
    ...
    const selectMenu = (parent, chartGroup) =>
